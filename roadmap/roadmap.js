...
function initializeRoadmapCards() {
    ...
    cards.forEach((card, index) => {
        // Ищем заголовок карточки для определения типа
        const title = card.querySelector('h3').textContent.trim();
        console.log(`Карточка ${index + 1}: "${title}"`);
        
        // Ищем соответствующую запись в данных
        let cardData = null;
        for (const key in roadmapData) {
            if (roadmapData[key].title === title) {
                cardData = roadmapData[key];
                console.log(`Найдены данные для: "${title}"`);
                break;
            }
        }
        ...
        if (cardData) {
            ...
            console.log(`Данные не найдены для: "${title}"`);
        }
    });
}
...
function updateRoadmapCardsLanguage(lang) {
    try {
        ...
        cards.forEach(card => {
            ...
            // Пытаемся получить id карточки из data-атрибута, иначе определяем по заголовку
            let cardId = card.getAttribute('data-roadmap-id');

            if (!cardId) {
                const currentTitle = titleEl.textContent.trim();
                // Ищем совпадение по RU/EN заголовкам через keys
                for (const key in roadmapData) {
                    const item = roadmapData[key];
                    const ruTitle = item.titleKey ? getTranslation(item.titleKey, 'RU') : item.title;
                    const enTitle = item.titleKey ? getTranslation(item.titleKey, 'EN') : item.title;
                    if (currentTitle === ruTitle || currentTitle === enTitle) {
                        cardId = item.id;
                        card.setAttribute('data-roadmap-id', cardId);
                        break;
                    }
                }
            }
            ...
            const item = Object.values(roadmapData).find(i => i.id === cardId);
            if (!item) return;
